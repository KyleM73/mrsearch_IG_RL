...
from mrsearch_IG_RL.models import ActorCriticICM
...
if __name__ == "__main__":
    ...
    if not args.zero_action:
        ...
        model = PPO.load(model_path,env,custom_objects={"CustomModel": ActorCriticICM})
    else:
    ...
    for _ in range(env.max_steps+1):
        if not args.zero_action:
            action, states, probs, intrinsic_rewards = model.predict(ob)
        else:
